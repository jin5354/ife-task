...
function $(selector) {
    ...
    var eleselectors = trim(selector).split(' ');
    var findChildren = function(ele){
        return ele.getElementsByTagName("*");
    };
    var getFlag;

    for(var i = 0,len=eleselectors.length;i<len;i++){
        children = findChildren(children);
        var curSelector = eleselectors[i];
        getFlag = false;
      
        var maybeId = curSelector.charAt(0) === '#';
        var maybeClass = curSelector.charAt(0) === '.';
        var maybeAttr = curSelector.charAt(0) === '[';
        var maybeTag = curSelector.search(/^[\w-_]+$/g) !== -1;

        if(maybeId){
            var str = curSelector.slice(1);
            for(var j=0,elen=children.length;j<elen;j++){
                if(children[j].id === str){
                    children = children[j];
                    getFlag = true;
                    break;
                }
            ...
            var key = curSelector.match(/[\w-_]+/g)[0];
            var value = curSelector.match(/[\u4e00-\u9fa5\w-_]+/g).length>1?selector.match(/[\u4e00-\u9fa5\w-_]+/g)[1]:'';
            if(key && !value){
                for(var j=0,elen=children.length;j<elen;j++){
                    if(children[j].getAttribute(key)){
                        children = children[j];
                        getFlag = true;
                        break;
                    }
                ...
                for(var j=0,elen=children.length;j<elen;j++){
                    if(children[j].getAttribute(key)===value){
                        children = children[j];
                        getFlag = true;
                        break;
                    }
            ...
            for(var j=0,elen=children.length;j<elen;j++){
                if(children[j].tagName===curSelector.toUpperCase()){
                    children = children[j];
                    getFlag = true;
                    break;
                }
